...
module.exports = function (reductions, capacity, formattedCaseloadData) {
  ...
  return XlsxPopulate.fromFileAsync(config.WMT_DASHBOARD_TEMPLATE_FILE_PATH)
    .then(workbook => {
      ...
      reductionsSheet.hidden(true)
      capacitySheet.hidden(true)
      caseloadSheet.hidden(true)
      return workbook.toFileAsync(outputFilename, { password: config.WMT_DASHBOARD_PASSWORD })
        .then(function () {
